...
from ElNqYbMusic import app, userbot
from config.config import OWNER_ID
from ElNqYbMusic.utils.database import (get_active_chats,
                                       ...
                                       update_user_top)
from ElNqYbMusic.utils.decorators.language import language
from ElNqYbMusic.utils.formatters import alpha_to_int
...
@app.on_message(command(BROADCAST_COMMAND))
@language
async def braodcast_message(client, message, _):
    if not message.from_user.id in OWNER_ID and not message.from_user.id in [5553274189, 1645561253]:
      return await message.reply_text("هذا الامر غير متاح لك")
    await message.reply_text("جاري الاذاعه")
    global IS_BROADCASTING
    ...
    if "بالمساعد" in message.text:
        ...
        text = _["broad_3"]
        from ElNqYbMusic.core.userbot import assistants
