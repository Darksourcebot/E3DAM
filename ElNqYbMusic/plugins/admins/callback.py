...
from ElNqYbMusic import YouTube, app
from ElNqYbMusic.core.call import ElNqYb
from ElNqYbMusic.misc import SUDOERS, db
from ElNqYbMusic.utils.database import (is_active_chat,
...
from ElNqYbMusic.utils.formatters import seconds_to_min
from ElNqYbMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from ElNqYbMusic.utils.stream.autoclear import auto_clean
from ElNqYbMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await ElNqYb.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await ElNqYb.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await ElNqYb.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await ElNqYb.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await ElNqYb.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
